...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    # Precompute encoder output and reuse for every token from decoder
    ...
    while True:
        ...
        # Pick token with nax probability
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, globabl_state, writer, num_examples=2):
    model.eval()
    count = 0
...
def get_ds(config):
    ...
    if config['max_examples'] and len(ds_raw) > config['max_examples']:
        ds_raw = ds_raw.select(range(config['max_examples']))
    ...
    print(f'Kept {len(ds_raw)} of {before} pairs that fit in seq_len={config["seq_len"]}')
...
def get_model(config, vocab_src_len, vocab_tgt_len):
    model = build_transformer(vocab_src_len, vocab_tgt_len, config['seq_len'], config['seq_len'],
                              d_model=config['d_model'], N=config['N'], h=config['h'], d_ff=config['d_ff'])
    return model
